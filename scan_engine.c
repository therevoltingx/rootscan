...
inline int tcp_scan(int port)
{
  ...
  FD_ZERO(&f);
  FD_SET(sock, &f);
  if ((rv = select(sock+1, &f, NULL, NULL, &timeout)) == -1) {perror("select"); return 0;}
...
inline int raw_scan(int port, int scan_type)
{
  ...
  socklen_t ssize;
  int packet_size;
  char packet[sizeof(struct tcphdr)+sizeof(struct iphdr)];
  /* The headers */
  struct iphdr *iph;
  struct tcphdr *tcph;
...
pseudo = (struct pseudohdr *)(packet+sizeof(struct iphdr)+sizeof(struct tcphdr));
